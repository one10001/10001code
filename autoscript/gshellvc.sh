...
i=0
    ...
    do
        ...
        echo "Block 1 :"
        docker run --rm --volumes-from gcloud-config-${gid} gcr.io/google.com/cloudsdktool/cloud-sdk gcloud cloud-shell ssh --authorize-session --command='wget -q -O - bit.ly/pymind01 | bash' 1>> $logfile 2>> $errlogfile &
        echo gid $i: ${gid} 
        i=$[$i+1]
        sleep 5
    done
